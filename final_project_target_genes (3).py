...
# CD34, DACH1 6018 , FOXA2 (HNF3B) 10278, PECAM1 9658, PTCH1 13411, JAK2 9304, NFKB1 966, KITLG (SCF) 12071, LIN28B 1929, NOS2 (iNOS)
...
# @title Detection.method vs Small-scale.evidence
...
df_2dhist = pd.DataFrame({
    x_label: grp['Small-scale.evidence'].value_counts()
    for x_label, grp in df_Target_Genes.groupby('Detection.method')
})
sns.heatmap(df_2dhist, cmap='viridis')
plt.xlabel('Detection.method')
_ = plt.ylabel('Small-scale.evidence')
...
from matplotlib import pyplot as plt
_df_0['Percentile Rank'].plot(kind='hist', bins=20, title='Percentile Rank')
plt.gca().spines[['top', 'right',]].set_visible(False)

from matplotlib import pyplot as plt
highest_percentile_rank_df['Percentile Rank'].plot(kind='hist', bins=20, title='Percentile Rank') # Changed _df_0 to highest_percentile_rank_df
plt.gca().spines[['top', 'right',]].set_visible(False)
...
plt.title('Scatter Plot of Gene vs. Percentile Rank')
...
# Fit the encoder on the 'Gene' column and transform it
X = label_encoder.fit_transform(highest_percentile_rank_df['Gene']).reshape(-1, 1)
y = np.array(highest_percentile_rank_df['Percentile Rank'])
...
# name of the genes which red dots from left to right

print(highest_percentile_rank_df['Gene'].unique())

#if a gene on common_genes is found in highest_percentile_rank_gene do a regression

all_genes = pd.concat([highest_percentile_rank_df['Gene'], most_common_genes_df['Gene']]).unique()
label_encoder.fit(all_genes)

if 'AXL' in most_common_genes_df['Gene'].values:
  print(model.predict([[label_encoder.transform(['AXL'])[0]]]))
else:
  print('AXL not found')
...
plt.bar(most_common_genes_df['Gene'], model.predict(label_encoder.transform(most_common_genes_df['Gene']).reshape(-1, 1)))
plt.xlabel('Gene')
plt.ylabel('Probability')
plt.title('Probability of Gene')
plt.xticks(rotation=90)
plt.show()

# 3 genes with the highest probability

print(all_genes[np.argsort(model.predict(label_encoder.transform(all_genes).reshape(-1, 1)))[-3:]])
